...
export function SlopeGame() {
  ...
  useEffect(() => {
    ...
    function createSegment(z: number) {
      ...
      group.position.z = z;
      scene.add(group);
      segments.push(group as unknown as THREE.Mesh);
    ...
    function resetGame() {
      ...
      isGameOver = false;
      isPlaying = true;
      setGameState({ score: 0, speed: 0, isPlaying: true, isGameOver: false });
    }
    ...
    function animate() {
      ...
      if (isPlaying && !isGameOver) {
        ...
        // Spawn new segments
        const lastSegmentZ =
          segments.length > 0 ? segments[segments.length - 1].position.z : 0;
      ...
      {!gameState.isPlaying && !gameState.isGameOver && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="text-center">
            ...
            <button
              onClick={() => {
                // Trigger space key behavior via state is handled by effect, use a ref approach
